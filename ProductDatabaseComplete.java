...
public class ProductDatabase extends JFrame implements ActionListener
{
   ...
   /**
    ...
    private void clearAllSales()
    {
        ...
        {
    		// Resets the value in the array to 0 
    		productSales[i] = 0;
        	percentage[i] = 0;
        }
    }
    ...
    private void computePercentages()
    {
        for (int i = 1; i <= actualProducts; i++)
        {
        	percentage[i] = (100 * productSales[i]) / (float)totalSales;		// Formula to convert sales value into a percentage
        }
    }
    ...
    {
    	if (actualProducts == MAX_PRODUCTS)
    	{
    		return false;
    	}
    ...
    {
    	totalSales -= productSales[productNumber];		
        productSales[productNumber] = prodSales;
        totalSales += prodSales;
        computePercentages();
    }
    ...
    {
    	if (deleteNumber == actualProducts)			// Checks if deleteNumber is last row
    	{
    		// Removes last row in use
    		productName[deleteNumber] = "";				
        	totalSales -= productSales[deleteNumber];		
        	productSales[deleteNumber] = 0;
        	percentage[deleteNumber] = 0;
        	computePercentages();
        	actualProducts--;			
    	}	
    	else		// if deleteNumber does not equal the last row 
    	{
    		// Shifts last row in use into row we want to remove
    		// Then we remove the last row
    		productName[deleteNumber] = productName[actualProducts];
        	totalSales -= productSales[deleteNumber];
        	productSales[deleteNumber] = productSales[actualProducts];
        	percentage[deleteNumber] = percentage[actualProducts];
        	computePercentages();
        	actualProducts--;
    	}
    ...
    {
        String nameTemp = "";		// Temporary values for productName and productSales
        int salesTemp = 0;
    	for (int i = 1; i <= actualProducts; i++)
        {
        	for(int j = i +1; j <=actualProducts; j++)
        	{
            if (productName[i].compareToIgnoreCase(productName[j]) > 0)		// Compares one word in the array to the word directly after it
            {
            	nameTemp = productName[i];
            	productName[i] = productName[j];
    			productName[j] = nameTemp;
    			salesTemp = productSales[i];
    			productSales[i] = productSales[j];
    			productSales[j] = salesTemp;
    			computePercentages();
            }
        	}
    ...
    private void sortBySalesDescending()
    {
        ...
        for (int i = 1; i <= actualProducts; i++)
        {
        	for (int j = i + 1; j <= actualProducts; j++)
        	{
        		if (productSales[i] < productSales[j])		// Compares one value in the array with the value directly after it to see if it is smaller
        		{
        			salesTemp = productSales[i];
        			productSales[i] = productSales[j];
        			productSales[j] = salesTemp;
